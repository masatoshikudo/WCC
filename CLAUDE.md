...
1. **Stripe Webhook 実装**（決済信頼性向上）— `docs/INFRA_SETUP.md` Stripe 項目と連動
2. **LP 問い合わせフォームの Supabase 接続**（`docs/LP_REVISION_PLAN.md` P1 項目）
3. **LP の BRAND.md 準拠見直し**（`docs/LP_REVISION_PLAN.md` P3 項目）
4. **SNS 実績アカウント立ち上げ**（Instagram / TikTok）
5. **`docs/INFRA_SETUP.md` 残項目消化**（LINE・SNS・GitHub branch 保護）
